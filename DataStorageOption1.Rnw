...
Utilizamos PDF/A porque nos interesa que los documentos PDF puedan conservarse largo tiempo. No queremos usar opciones que limiten la capacidad de archivo.

Aquí se dice como... \url{https://webpages.tuni.fi/latex/pdfa-guide.pdf}. En esta variante quiero usar exclusivamente hyperref, por lo que cedo todos los datos a hypersetup sin usar un ficheroe externo.
...
\item Sin embargo, se necesitan herramientas como Adobe Adrobat para poder acceder a los datos. Acrobat Reader visualiza los datos, pero no permite su extracción. Adobre Acrobat si lo permite, como puede observarse en la Fig.~\ref{fig:Option1VisibleAdobeAcrobat}.
...
\item \textbackslash hypersetup\{\} sólo se puede usar en el preámbulo, lo que es un problema, ya que los datos pueden estar disponibles al principio o no (bien es cierto que típicamente si lo están).

\item Lo que ya no parece posible es almacenar datos que correspondan a distintos estadios ... \textcolor{red}{Completar}.
